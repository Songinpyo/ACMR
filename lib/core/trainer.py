...
from mmaction2.demo import slowfast_raw, make_model
...
class Trainer():
    def __init__(
        ...
        self.train_2d_loader, self.train_3d_loader, self.valid_loader, self.action_train_3d_loader = data_loaders

        # exclude motion discriminator
        # self.disc_motion_iter = iter(self.disc_motion_loader)

        self.train_2d_iter = self.train_3d_iter = self.action_train_3d_iter = None
        ...
        if self.action_train_3d_loader:
            self.action_train_3d_iter = iter(self.action_train_3d_loader)
        ...
        self.slowfast = make_model()
        self.action_preprocessd = True
    ...
    def train(self):
        ...
        for i in range(self.num_iters_per_epoch):
            ...
            target_2d = target_3d = action_target_3d = None
            ...
            # target_2d = {dict:2}
            #   'features' = {tensor : (19, 16, 2048)
            #   'kp_2d' = {tensor : (19, 3, 49, 3)}
            ...
            if self.train_3d_iter:
                ...
                # target_3d = {dict:6}
                #   'features' = {tensor : (13, 16, 2048)
                #   'theta' = {tensor : (13, 3, 85)}
                #   'kp_2d' = {tensor : (13, 3, 49, 3)}
                #   'kp_3d' = {tensor : (13, 3, 49, 3)}
                #   'w_smpl' = {tensor : (13, 3)} [[0,0,0], [0,0,0], ...] // why all 0
                #   'w_3d' = {tensor : (13, 3)} [[1,1,1], [1,1,1], ...] // why all 1
                #   -------------------- inpyo revise --------------------------------
                #   'instance_id' = {list : 16}
                #       00 = {tuple : 13} [a0, b0, c0, ...]
                #       01 = {tuple : 13} [a1, b1, c1, ...]
                #       ...
                #       15 = {tuple : 13} [a15, b15, c15, ...]
                #       a00 = 'outdoors_slalom_00_0_00151'
                #   'imgname' = {list : 16}
                #       00 = {tuple : 13} [a0, b0, c0, ...]
                #       01 = {tuple : 13} [a1, b1, c1, ...]
                #       ...
                #       15 = {tuple : 13} [a15, b15, c15, ...]
                #       a00 = './data/3dpw/imageFiles/outdoors_slalom_00/image_00151.jpg'

            if self.action_train_3d_iter:
                try:
                    action_target_3d = next(self.action_train_3d_iter)
                except StopIteration:
                    self.action_train_3d_iter = iter(self.action_train_3d_loader)
                    action_target_3d = next(self.action_train_3d_iter)
                move_dict_to_device(action_target_3d, self.device)
            ...
            inp = action_target_3d['features'].cuda()
            if self.action_preprocessd:
                try:
                    top1_action_labels = action_target_3d['top1_action_label']
                    for i in range(len(top1_action_labels)):
                        top1_action_label.append(action_target_3d['top1_action_label'][i])
                except:
                    pass
            else:
                image_names = action_target_3d['imgname']
                image_paths = []
                start_indexes = []
                for i in range(len(image_names[0])):
                    start_image_name = image_names[0][i]

                    image_path = start_image_name.split('/')[0:5]
                    image_path = '/'.join(image_path) + '/'
                    image_paths.append(image_path)

                    start_index = start_image_name.split('/')[-1]
                    start_index = re.sub(r'[^0-9]', '', start_index)
                    start_index = start_index[-6:]
                    start_indexes.append(start_index)

                top1_action_labels, top2_action_labels, top3_action_labels = slowfast_raw(self.slowfast,
                                                                                            image_paths,
                                                                                            start_indexes)

                for i in range(len(top1_action_labels)):
                    top1_action_label.append(top1_action_labels[i][0])


            timer['data'] = time.time() - start
            start = time.time()

            preds, scores, action_pred = self.generator(inp, top1_action_label, is_train=True)
            ...
            gen_loss, loss_dict = self.criterion(
                ...
                data_2d=False,
                data_3d=action_target_3d,
                scores=scores,
                action_pred=action_pred,
                action_label=top1_action_label
            )
            ...
            # Action Accuracy
            if top1_action_label != []:
                correct = 0
                for i in range(len(top1_action_labels)):
                    if torch.Tensor(top1_action_label).long()[i] == torch.Tensor(action_pred).argmax(dim=1)[i]:
                        correct += 1

                action_acc = correct/len(top1_action_labels)
            else:
                action_acc = 0
