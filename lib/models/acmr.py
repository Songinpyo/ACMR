...
from lib.core.loss import AdaCos
...
class TemporalEncoder(nn.Module):
    ...
    def forward(self, x, is_train=False):
        ...
        # x_aft = {32, 7, 2048}, x_bef = {32, 8, 2048}
...
class ACMR(nn.Module):
    def __init__(
        ...
        self.adacos = AdaCos(in_features=2048, out_features=400, m=1.0)
    ...
    def forward(self, input, action_labels=None, is_train=False, J_regressor=None):
        # input size NTF
        # input = {tensor : (32, 16, 2048)} == (batch, seq_len, feature)
        batch_size, seqlen = input.shape[:2]

        # feature = {tensor : (32, 3, 2048)} // scores = {tensor : (32, 3)}
        # past, currnet, next
        feature, scores = self.encoder(input, is_train=is_train)

        action_pred = None
        if is_train and action_labels:
            current_feature = feature[:,1,:]
            # current_feature = self.action_classifier(current_feature)
            action_pred = self.adacos(current_feature.squeeze(), torch.Tensor(action_labels).to('cuda'))
        ...
        if is_train:
            return smpl_output, scores, action_pred
        else:
