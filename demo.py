...
from lib.models.acmr import ACMR
...
def main(args):
    ...
    image_folder = args.vid_file
    num_frames = len(os.listdir(image_folder))
    img_shape = cv2.imread(osp.join(image_folder, 'image_00001.jpg')).shape
    orig_height, orig_width = img_shape[:2]
    ...
    """ Run ACMR on each person """
    print("\nRunning ACMR on each person tracklet...")
    ACMR_time = time.time()
    ACMR_results = {}
    for person_id in tqdm(list(tracking_results.keys())):
        ...
        ACMR_results[person_id] = output_dict

    del model

    end = time.time()
    fps = num_frames / (end - ACMR_time)
    print(f'ACMR FPS: {fps:.2f}')
    total_time = time.time() - total_time
    print(f'Total time spent: {total_time:.2f} seconds (including model loading time).')
    print(f'Total FPS (including model loading time): {num_frames / total_time:.2f}.')

    if args.save_pkl:
        print(f"Saving output results to \'{os.path.join(output_path, 'ACMR_output.pkl')}\'.")
        joblib.dump(ACMR_results, os.path.join(output_path, "ACMR_output.pkl"))
    ...
    frame_results = prepare_rendering_results(ACMR_results, num_frames)
    # mesh_color = {k: colorsys.hsv_to_rgb(np.random.rand(), 0.75, 0.5) for k in ACMR_results.keys()}
    mesh_color = {k: colorsys.hsv_to_rgb(0.36, 0.5, 1.0) for k in ACMR_results.keys()}
    ...
    save_name = f'ACMR_{vid_name.replace(".mp4", "")}_output.mp4'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='./data/base_data/ACMR_demo_model.pth.tar', help='path to pretrained model weight')
